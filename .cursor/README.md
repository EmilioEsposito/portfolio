...
- **ports**: Exposed ports for NextJS (3000) and FastAPI (8000)
...
2. Updates Playwright browsers to match project version (fast, since base browsers are in snapshot)
3. Installs Python project dependencies with `uv sync`
4. Starts PostgreSQL service
5. Creates database user and database
6. Runs database migrations with Alembic
7. Configures PYTHONPATH
...
- **What**: System-level tools (Node.js, Python, pnpm, uv, PostgreSQL, Playwright + browsers)
...
3. Three terminal tabs open automatically:
   - **NextJS**: Development server on port 3000
