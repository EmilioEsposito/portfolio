...
- **Outbound HTTPS is TLS-intercepted (MITM) by Anthropic's egress gateway.** Every host's cert is re-signed by `O=Anthropic, CN=Egress Gateway SDS Issuing CA`, whose root lives **only** in the system bundle (`/etc/ssl/certs/ca-certificates.crt`, exported as `SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE`). Two gotchas this creates:
  - **Libraries with their own bundled CA fail** even though `curl`/`httpx`/`requests` (system bundle) work. The main offender is **`httplib2`** (used by `google-api-python-client`) → Gmail/Sheets/Calendar/Drive raise `CERTIFICATE_VERIFY_FAILED: self-signed certificate in certificate chain`. Fixed centrally by `api/src/google/common/ca_bundle.py` (sets `HTTPLIB2_CA_CERTS` + `httplib2.CA_CERTS` to the system bundle; no-op/safe in prod). If you add another client that bundles its own CA, point it at `SSL_CERT_FILE` the same way. (Note: uv's `--native-tls`/`UV_NATIVE_TLS` only governs uv's *own* package downloads, not the app runtime — it can't fix this.)
  - **OpenPhone/Quo direct HTTP is unreachable** — `api.openphone.com` and `api.quo.com` (same Cloudflare edge) reject the gateway's *upstream* TLS handshake with `TLSV1_ALERT_PROTOCOL_VERSION` (their edge enforces stricter TLS than the gateway offers; other Cloudflare hosts work, so it's vendor-specific, **not** a local trust issue and not fixable in app code). Use the **Quo MCP server** (separate egress path) for contact/message/call lookups in-sandbox. Production Railway has direct egress and is unaffected.
...
- **Test location**: `api/src/tests/`. ALL tests live there — do not embed `test_*` functions inside service modules (that pattern was consolidated away in 2026-07; collection is standard `test_*.py` files only).

### Pytest Gotchas (learned the hard way)

- Collection is standard (`python_files = test_*.py *_test.py`, tests in `api/src/tests/`). It used to be `*.py`, which collected tests out of every service module and forced workarounds (`__test__ = False` on FastAPI handlers named `test_*`, `__init__.py` in every package dir to avoid double-import model registration). Don't loosen it back, and avoid naming route handlers `test_*` regardless.
