...
async def get_zillow_emails(db: AsyncSession = Depends(get_db)):
    """Get a sample of Zillow rental inquiry emails."""
    query = select(EmailMessage).where(
        and_(
            EmailMessage.subject.ilike("%zillow%"),
            EmailMessage.subject.ilike("%rental%")
        )
