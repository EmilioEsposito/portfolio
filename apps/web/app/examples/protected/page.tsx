...
export default function ProtectedExample() {
  ...
  const [serniacapitalData, setSerniacapitalData] = useState<string | null>(null);
  const [serniacapitalGetUserData, setSerniacapitalGetUserData] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [googleError, setGoogleError] = useState<string | null>(null);
  const [simpleError, setSimpleError] = useState<string | null>(null);
  const [serniacapitalError, setSerniacapitalError] = useState<string | null>(null);
  const [serniacapitalGetUserError, setSerniacapitalGetUserError] = useState<string | null>(null);
  ...
  const [isSerniacapitalGetUserLoading, setIsSerniacapitalGetUserLoading] = useState(false);
  ...
  const fetchProtectedSerniacapitalData = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        } catch (e) { /* If parsing JSON fails, use the status text */ }
        throw new Error(`${response.status}. ${errorDetail}`);
      }
      ...
      setSerniacapitalError(err instanceof Error ? err.message : 'An error occurred');
    } finally {
  ...
  const fetchProtectedSerniacapitalGetUserData = async () => {
    ...
    try {
      const token = await getToken();
      const response = await fetch('/api/examples/protected_serniacapital_or_admin', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      setSerniacapitalGetUserError(err instanceof Error ? err.message : 'An error occurred');
    } finally {
    ...
    <div className="container mx-auto py-10 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            This example demonstrates accessing a protected FastAPI endpoint that only requires authentication.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            onClick={fetchProtectedSimpleData}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            This example demonstrates accessing a protected FastAPI endpoint that requires Sernia Capital domain verification.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
          ...
          >
            {isSerniacapitalLoading ? 'Loading...' : 'Fetch Sernia Capital Data'}
          </Button>

          {serniacapitalError && (
            <div className="text-red-500 mt-4">
              Error: {serniacapitalError}
            </div>
          )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            This example demonstrates accessing a protected FastAPI endpoint that requires Sernia Capital domain or admin verification.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Input 
          ...
          >
            {isSerniacapitalGetUserLoading ? 'Loading...' : 'Fetch Sernia Capital Admin/User Data'}
          </Button>
