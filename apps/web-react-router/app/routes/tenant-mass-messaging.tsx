import type { Route } from "./+types/tenant-mass-messaging";
...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "Tenant Mass Messaging | Emilio Esposito" },
    {
      name: "description",
      content: "Send SMS messages to all tenants in selected properties",
    },
  ];
}
    ...
    try {
      ...
      if (response.status === 401) {
        setStatus({
          type: "error",
          message:
            data.detail || "Authentication failed. Please check your password.",
        });
        return;
      }
      ...
      if (!response.ok) {
        throw new Error(
          data.detail || data.message || data.error || "Failed to send message"
        );
      }
      ...
      // Fallback error
      throw new Error(
        data.message || data.error || data.detail || "Failed to send message"
      );
    } catch (error) {
    ...
    <>
      <div className="max-w-md mx-auto mt-10 p-6 bg-card text-card-foreground rounded-lg shadow-lg">
        ...
        <div className="mb-6 text-muted-foreground">
          <p className="text-sm">
            This tool allows property managers to send SMS messages to all
            tenants in selected properties. This is useful for notifying tenants
            of a building-wide event like a water leak, power outage, or common
            area maintenance.
          </p>
          <br />
          <p className="text-sm">
            Since OpenPhone doesn't support mass messaging, this tool uses their
            API to send individual messages to each tenant. Messages are sent
            securely and require password authentication.
          </p>
        </div>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {propertyNames.length} property(s)?
              </DialogDescription>
            </DialogHeader>
      ...
      <div className="max-w-md mx-auto mt-6 p-4 text-sm text-muted-foreground">
        ...
        <ul className="space-y-2">
          ...
          <li>
            <a
              href="https://github.com/EmilioEsposito/portfolio/blob/main/apps/web-react-router/app/routes/tenant-mass-messaging.tsx"
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary hover:underline flex items-center"
            >
              Frontend Implementation (React Router)
            </a>
          </li>
        </ul>
      </div>
