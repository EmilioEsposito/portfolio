...
from apps.api.src.open_phone.service import send_message, upsert_openphone_contact
from apps.api.src.database.database import AsyncSessionFactory
from apps.api.src.scheduler.service import scheduler
from apps.api.src.contact.service import ContactCreate
...
import openai
import pytz
from apps.api.src.google.calendar.service import create_calendar_event, get_calendar_service
...
async def check_email_threads(overwrite_calendar_events=False):
    ...
    async with AsyncSessionFactory() as session:
        ...
        with open("apps/api/src/zillow_email/zillow_email_threads.sql", "r") as f:
            sql_query = f.read()
...
async def start_service():
    ...
    scheduler.add_job(
        ...
        kwargs={
            "sql": "apps/api/src/zillow_email/test.sql",
            "target_slugs": ["emilio"],
        },
    ...
    scheduler.add_job(
        ...
        func=check_unreplied_emails,
        kwargs={
            "sql": "apps/api/src/zillow_email/zillow_email_new_unreplied.sql",
            "target_slugs": ["sernia"],
        },
