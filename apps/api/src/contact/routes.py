...
from apps.api.src.contact import service as contact_service
from apps.api.src.contact.service import ContactCreate, ContactResponse, ContactUpdate
...
async def create_new_contact(
    ...
    """
    logfire.info(f"Attempting to create new contact with slug: {contact.slug}")
    try:
        created_contact = await contact_service.create_contact(db=db, contact_create=contact)
        logfire.info(f"Successfully created contact with ID: {created_contact.id}")
        return created_contact
    except HTTPException as e:
        logfire.error(f"Error creating contact: {e.detail}")
        raise
...
async def read_all_contacts(
    ...
    """
    logfire.info(f"Attempting to read all contacts with skip: {skip}, limit: {limit}")
    try:
        contacts = await contact_service.get_all_contacts(db, skip=skip, limit=limit)
        logfire.info(f"Successfully retrieved {len(contacts)} contacts.")
        return contacts
    except Exception as e:
...
async def read_contact_by_id(
    ...
    """
    logfire.info(f"Attempting to read contact by ID: {contact_id}")
    try:
        ...
        logfire.info(f"Successfully retrieved contact with ID: {contact_id}")
        return db_contact
    except HTTPException as e:
...
async def read_contact_by_slug(
    ...
    """
    logfire.info(f"Attempting to read contact by slug: {slug}")
    try:
        ...
        if e.status_code != status.HTTP_404_NOT_FOUND:
            logfire.error(f"Error reading contact by slug {slug}: {e.detail}")
        raise
    except Exception as e:
...
async def update_existing_contact(
    ...
    try:
        ...
        if updated_contact is None:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
        logfire.info(f"Successfully updated contact with ID: {contact_id}")
