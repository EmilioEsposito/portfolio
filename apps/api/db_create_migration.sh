...
code apps/api/src/database/migrations/versions/$(ls apps/api/src/database/migrations/versions/ | grep -E "^\d{4}_.*\.py$" | tail -1)
...
if [ "$commit_migration" = "y" ]; then
    ...
    echo "Migration not committed. Run 'sh apps/api/db_run_migration.sh' to commit the migration"
fi
