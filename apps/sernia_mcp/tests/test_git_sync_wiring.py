"""Tests for the git_sync wiring around ``edit_resource`` and app startup.
...
two integration points:
  ...
  1. ``edit_resource`` schedules a fire-and-forget ``commit_and_push`` task.
...
async def test_edit_resource_schedules_commit_and_push():
    """A successful ``edit_resource`` call should kick off ``commit_and_push``."""
...
@pytest.mark.asyncio
async def test_edit_resource_skips_git_sync_on_failure():
    """If ``write_memory`` fails, no commit_and_push should fire."""
    from fastmcp import Client
