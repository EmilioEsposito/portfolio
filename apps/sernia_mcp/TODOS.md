...
### Group-thread message history — resolved September 10, 2026

Quo's June 18 API update supports group history via repeated unbracketed
`participants` keys. The reader now uses that endpoint and validates conversationId.
The prior backend proxy / database integration proposal is no longer needed.
API failures still return explicitly labeled partial context.
