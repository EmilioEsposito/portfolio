...
async def seed_contacts(interactive: bool = False, dry_run: bool = False) -> None:
    ...
    async with AsyncSessionFactory() as session:
        for seed in CONTACT_SEEDS:
            ...
            if existing:
                logfire.info(f"✓ Contact '{seed.slug}' already exists (id: {existing.id})")
                continue
            ...
            logfire.info(f"should_prompt: {should_prompt}")

            if should_prompt and len(seed.interactive_fields) > 0:
                logfire.info(f"Interactive mode for '{seed.slug}':")
            ...
            try:
                ...
                logfire.info(f"✓ Created contact '{seed.slug}' (id: {created.id})")
            except Exception as e:
                logfire.error(f"✗ Failed to create contact '{seed.slug}': {e}")
                raise
...
async def main(interactive: bool = False, dry_run: bool = False) -> None:
    ...
    logfire.info("=" * 50)
    logfire.info("Database Seed Script")
    logfire.info("=" * 50)

    if dry_run:
        logfire.info("Running in DRY RUN mode - no changes will be made")
    if interactive:
        logfire.info("Running in INTERACTIVE mode - will prompt for values")

    logfire.info("")
    logfire.info("Seeding contacts...")
    await seed_contacts(interactive=interactive, dry_run=dry_run)

    logfire.info("")
    logfire.info("=" * 50)
    logfire.info("Seed complete!")
    logfire.info("=" * 50)
