...
async def protected_route_get_session(session: Annotated[Session, Depends(get_auth_session)]):
    """
    Example protected endpoint that requires authentication.
    Returns user information from Clerk.
    """
...
async def protected_route_get_user(user: AuthUser):
    ...
    data = {"user_email": user_email, "user": user, "dummy_data": "dummy"}
    return data
...
@router.post(
    "/protected_serniacapital_or_admin", dependencies=[Depends(verify_admin_or_serniacapital)]
)
...
async def protected_route_google(
    request: Request, credentials: Annotated[Credentials, Depends(get_google_credentials)]
):
    """
    ...
    debug_info = {
        ...
        "time_until_expiry": (credentials.expiry - current_time).total_seconds()
    ...
    try:
        ...
        return {"error": f"Failed to access Gmail API: {str(e)}", "credential_info": debug_info}
