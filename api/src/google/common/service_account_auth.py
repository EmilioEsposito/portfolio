    ...
    try:
        ...
        try:
            ...
            if creds_dict["type"] != "service_account":
                raise HTTPException(
                    status_code=500, detail="Invalid credentials type. Must be 'service_account'"
                )
        ...
        credentials = service_account.Credentials.from_service_account_info(
            creds_dict, scopes=scopes or SERVICE_ACCOUNT_DEFAULT_SCOPES
        )
...
def get_delegated_credentials(
    user_email: str, scopes: list[str] | None = None
