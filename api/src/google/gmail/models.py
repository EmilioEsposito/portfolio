...
class EmailMessage(Base):
    ...
    from_address: Mapped[str] = mapped_column(
        String
    )  # Using from_address since 'from' is a reserved word
    ...
    raw_payload: Mapped[dict] = mapped_column(
        JSON
    )  # Store the full message payload for future processing
    # New columns
    first_history_id: Mapped[int] = mapped_column(
        Integer, nullable=True
    )  # First history ID when message was seen
    history_ids: Mapped[list[int]] = mapped_column(
        ARRAY(Integer), nullable=True
    )  # Array of history IDs where this message appeared
    label_ids: Mapped[list[str]] = mapped_column(
        ARRAY(String), nullable=True
    )  # Array of Gmail label IDs
