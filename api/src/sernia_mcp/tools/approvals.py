...
- Direct calls via tools/call raise "Unknown tool" (structural isolation
  verified in tests).
- Even if a caller somehow knew the name, they'd need a valid UUID from
  the pending dict, which is only surfaced through the UI.

Clients that can't render the PrefabApp (pydantic-ai, raw MCP clients,
terminals) will receive the structured PrefabApp payload back but have no
way to reach the confirm tool — the send cannot happen. Enforcement is
purely structural, not a capability check. This was an explicit design
choice after finding that `ctx.client_supports_extension(UI_EXTENSION_ID)`
returns False even for the `fastmcp dev apps` dev UI, because the MCP-level
client (the Python proxy) doesn't advertise the extension even though the
browser on top IS an Apps host.

State (`_PENDING`) is in-memory for the POC. A restart loses pending
approvals. Each row has a timestamp; rows older than `_PENDING_TTL_SECONDS`
are rejected on confirmation to prevent orphaned entries from being
approved much later. DB-backed durability is a later concern.
"""
import time
import uuid
...
from api.src.sernia_ai.config import INTERNAL_EMAIL_DOMAIN
from api.src.sernia_mcp.identity import resolve_user_email_for_request
from api.src.tool_core.errors import CoreError, NotFoundError, ValidationError
from api.src.tool_core.google.gmail import send_email_core
from api.src.tool_core.quo.send_sms import resolve_sms_routing_core, send_sms_core
...
async def quo_send_sms(to_phone: str, message: str) -> PrefabApp:
    ...
    """
    # Resolve routing up front — also validates the contact exists in Quo.
...
async def google_send_email(to: list[str], subject: str, body: str) -> PrefabApp:
    ...
    audience = "INTERNAL (all @serniacapital.com)" if all_internal else "EXTERNAL"
    with Card(css_class="max-w-xl") as view:
        with CardHeader():
