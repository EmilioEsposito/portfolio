"""SMS routing + send core functions for Quo (OpenPhone).

The routing logic (internal contact → AI line, external → shared team number)
mirrors sernia_ai/tools/quo_tools.py — the intent is that callers resolve
routing first, decide whether to proceed (e.g. reject external sends from
MCP), then call ``send_sms_core`` to actually send.
"""
import re
...
from api.src.open_phone.service import find_contact_by_phone
...
from api.src.tool_core.errors import ExternalServiceError, NotFoundError, ValidationError
from api.src.tool_core.quo._client import build_quo_client
from api.src.tool_core.types import SmsResult, SmsRouting
...
def _split_sms(text: str, limit: int = SMS_SPLIT_THRESHOLD) -> list[str]:
    """Split at sentence/newline boundaries, matching sernia_ai's split_sms()."""
    if len(text) <= limit:
        return [text]
    chunks: list[str] = []
    remaining = text
...
async def send_sms_core(
    ...
    *,
    ...
    """Send a single SMS via Quo. Caller is responsible for approval gating.

    If ``routing`` is not provided, it is resolved from the phone number.
    Auto-splits messages between SMS_SPLIT_THRESHOLD and SMS_MAX_LENGTH.
    Rejects messages longer than SMS_MAX_LENGTH.
    """
