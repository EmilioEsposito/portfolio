...
if not DEV_CLERK_WEBHOOK_SECRET:
    logger.warning("DEV_CLERK_WEBHOOK_SECRET environment variable not set. Webhook verification will fail.")
...
if not PROD_CLERK_WEBHOOK_SECRET:
    logger.warning("PROD_CLERK_WEBHOOK_SECRET environment variable not set. Webhook verification will fail.")
    # Depending on your policy, you might want to raise an error here
...
async def handle_clerk_webhook(
    ...
    if not webhook_dev or not webhook_prod:
        logger.error("One or more webhook secrets are not configured.")
    ...
    try:
        logger.debug("Attempting verification with DEV secret.")
        event = webhook_dev.verify(payload_str, headers)
        environment = "development"
        logger.info(f"Webhook verified successfully with DEV secret. Event type: {event.get('type')}")
    except WebhookVerificationError as e_dev:
        logger.warning(f"Webhook verification failed with DEV secret: {e_dev}. Trying PROD secret.")
        try:
            event = webhook_prod.verify(payload_str, headers)
            environment = "production"
            logger.info(f"Webhook verified successfully with PROD secret. Event type: {event.get('type')}")
        except WebhookVerificationError as e_prod:
            logger.error(f"Webhook verification failed with both DEV and PROD secrets: DEV_Error='{e_dev}', PROD_Error='{e_prod}'")
            raise HTTPException(status_code=400, detail="Webhook verification failed")
        except Exception as e:
            # Catch other errors during PROD verification attempt
            logger.error(f"Unexpected error during PROD webhook verification: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail="Internal server error during verification")
    except Exception as e:
        # Catch other errors during DEV verification attempt
        logger.error(f"Unexpected error during DEV webhook verification: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error during verification")
    ...
    if event is None or environment is None:
        ...
        logger.error("Webhook event object or environment is None after verification attempts.")
    ...
    try:
        if event_type == "user.created" or event_type == "user.updated":
            logger.info(f"Processing {event_type} for user: {event_data.get('id')} from env: {environment}")
            ...
            logger.info(f"Received unhandled event type: {event_type} from env: {environment}")
        ...
        logger.error(f"Value error processing webhook: {ve}")
    ...
    logger.info(f"Webhook processing completed. Result: {message}")
