...
async def upsert_user(db: AsyncSession, event_data: dict, environment: str):
    ...
    stmt = select(User).where(User.clerk_user_id == clerk_user_id, User.environment == environment)
    ...
    if existing_user:
        ...
        try:
            ...
            if existing_user:
                logfire.warn(
                    f"Found user {existing_user.id} after IntegrityError, proceeding as update."
                )
...
async def delete_user(db: AsyncSession, event_data: dict, environment: str):
    ...
    stmt = delete(User).where(User.clerk_user_id == clerk_user_id, User.environment == environment)
    result = await db.execute(stmt)
    message = ""
